...
const Order = () => {
    ...
    const options = [
        ...
        {
            id: 4,
            title: "Sugar Care Tea 200 Gram Pack",
            description: "1 Premium 200g Pack (Stevia & Spices)",
            price: 349,
            saving: "HEALTHY CHAI",
            badge: "NEW LAUNCH",
            image: "/sugar-care-tea.webp"
        }
    ];
    ...
    useEffect(() => {
        window.scrollTo(0, 0);
        // Trigger Google Tag Begin Checkout Event
        if (currentPack && typeof window.gtag === 'function') {
            window.gtag('event', 'begin_checkout', {
                value: currentPack.price * qty,
                currency: 'INR',
                items: [{
                    item_name: currentPack.title,
                    quantity: qty,
        ...
        const options_rzp = {
            ...
            handler: function (response) {
                const orderDetails = {
                    ...
                    // Flattened fields for Google Sheets mapping
                    name: formData.name,
                    phone: formData.phone,
                    address: formData.address,
                    city: formData.city,
                    pincode: formData.pincode,
                    // Keep nested object for front-end compatibility
                    customer: formData
                };
                ...
                // Send to Google Sheet (using both query params and JSON body for 100% compatibility with e.parameter and e.postData.contents)
                ...
                fetch(`https://script.google.com/macros/s/AKfycby8TUU16Bu7p4la3WSOjlSh2r66jbE0yxYYz7ZdZ7_nIGXEjsX4lkglqLQLfO55vFgM/exec?${queryParams.toString()}`, {
                    method: 'POST',
                    mode: 'no-cors',
                    cache: 'no-cache',
                    headers: {
    ...
    return (
        <div className="bg-surface font-body text-on-surface min-h-screen">
            <Helmet>
                <title>Secure Checkout | PohaFix Order</title>
                <meta name="description" content="Securely complete your PohaFix order. Experience 100% natural, fast, and authentic Indori Poha delivered right to your doorstep." />
            </Helmet>
            <Header />
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className="w-16 h-16 bg-surface-container-low rounded-lg overflow-hidden flex items-center justify-center p-2">
                                                <img src={opt.image} alt={opt.title} loading="eager" fetchpriority="high" className="w-full h-full object-contain" />
                                            </div>
                                    ...
                                    <div className="space-y-6 mb-8">
                                        <div className="flex gap-4 items-center">
                                            <div className="relative w-24 h-24 flex-shrink-0 bg-surface-container-low rounded-md overflow-hidden flex items-center justify-center p-2">
                                                <img src={currentPack.image.replace(/\.(png|jpg)$/, '.webp')} alt="Selected Pack" className="w-full h-full object-contain mix-blend-multiply" loading="eager" fetchpriority="high" />
